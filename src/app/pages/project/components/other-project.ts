    ...
    @defer(on viewport) {
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-screen-lg mx-auto my-10 text-left">
        @for (post of projectService.otherProjects(); track post.id) {
          <div class="dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 py-8 px-7 flex justify-between flex-col items-start">
            <header class="w-full">
              ...
              <p class="mb-2 text-lg font-semibold tracking-tight text-gray-900 dark:text-gray-300 group-hover:text-primary-500">
                {{post.title}}</p>
              <p class="mb-2 text-sm text-gray-900 dark:text-gray-200">{{post.description}}</p>
            </header>
            <footer class="mt-4 flex flex-row gap-2">
              <ng-container *ngFor="let t of post.tools; trackBy: trackByIndex">
                <devicon [name]="t" cssClass="w-5"></devicon>
              </ng-container>
            </footer>
          </div>
        }
